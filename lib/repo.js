...
function followAllFiles(repoPath, files) {
  const set = new Set(files);

  for (const file of files) {
    const cmd = `git log --name-only --pretty=format: --follow -- ${file}`;
    const res = childProcess.execSync(cmd, { cwd: repoPath, stdio: 'pipe' });
    const allPathsOfFile = res
      .toString()
      .split('\n')
      .filter(Boolean);
    for (const f of allPathsOfFile) {
      set.add(f);
    }
  }
...
function filter(repoPath, files) {
  ...
  log('Getting all renames of files');
  const filesWithRenames = followAllFiles(repoPath, files);
  console.log(filesWithRenames);
  log(`Found ${filesWithRenames.length} matching files`);
  const fileString = filesWithRenames.join(' ');
